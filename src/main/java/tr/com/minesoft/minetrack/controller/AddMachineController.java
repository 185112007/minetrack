	...
	private void updateTableRow(final DefaultTableModel model) {
		...
		if (selectedRow >= 0) {
			...
			if ((mno == mnoNew)&&(!fname.equals(fnameNew) || !lname.equals(lnameNew) || !role.equals(roleNew)
				...
				if (MachineList.getInstance().update(new Machine(mnoNew, fnameNew, lnameNew, roleNew, tidNew), tid)) {
				...
				JOptionPane.showMessageDialog(parent.getComponent(0),
						Messages.getString("AddMachineController.nochange_or_mnochanged"), //$NON-NLS-1$
						Messages.getString("AddMachineController.warning"), //$NON-NLS-1$
						JOptionPane.INFORMATION_MESSAGE);
			}
